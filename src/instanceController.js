...
function instanceController(  eventBus, 
                              clarinetParser, contentBuilderHandlers) {
                                
   eventBus(STREAM_DATA).on( clarinetParser.write.bind(clarinetParser));      
   
   /* At the end of the http content close the clarinet parser.
      This will provide an error if the total content provided was not 
      valid json, ie if not all arrays, objects and Strings closed properly */
   eventBus(STREAM_END).on( clarinetParser.close.bind(clarinetParser));
