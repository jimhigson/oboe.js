(function (Platform) {
  ...
  describe('oboe integration (real http)', function () {
    ...
    it('emits error with incompleteObject json', function (done) {
      oboe(testUrl('static/json/incompleteObject.json'))
        .fail(function (err) {
          expect(err.thrown.message).toMatch('Unexpected end')
          done()
        })
        ...
        toHaveBeenGivenAnyError: function () {
          return {
            compare: function (actual, expected) {
              ...
              var maybeErr = errorReport.thrown
              ...
              if (window && window.XMLHttpRequestException &&
                maybeErr instanceof window.XMLHttpRequestException) {
                return { passed: true }
              }
